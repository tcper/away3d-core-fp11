package away3d.extrusions
{
  ...
  */
  public class LatheExtrude extends Mesh
  {
    ...
    */
    public function LatheExtrude(  material : MaterialBase = null, profile:Vector.<Vector3D> = null, axis:String = LatheExtrude.Y_AXIS , revolutions:Number = 1, subdivision:uint = 10, coverall:Boolean = true, centerMesh:Boolean = false,
        flip:Boolean = false,  thickness:Number = 0, preciseThickness:Boolean = true, offsetRadius:Number = 0, ignoreSides:String = "", tweek:Object = null, smoothSurface:Boolean = true)
    {
      var geom : Geometry = new Geometry();
      _subGeometry = new SubGeometry();

      //if(!material && null) material = materials.front;
      super(geom, material);
    ...
    private function generate(vectors:Vector.<Vector3D>, axis:String, tweek:Object, render:Boolean = true, id:uint = 0):void
    {
      ...
      if (isNaN(tweek[X_AXIS]) || !tweek[X_AXIS]) tweek[X_AXIS] = 0;
      if (isNaN(tweek[Y_AXIS]) || !tweek[Y_AXIS]) tweek[Y_AXIS] = 0;
      if (isNaN(tweek[Z_AXIS]) || !tweek[Z_AXIS]) tweek[Z_AXIS] = 0;
